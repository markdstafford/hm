...
2. Each direct drag-active title-bar spacer in `AppShell.tsx` also calls `startWindowDragFromPointerEvent` from `src/shell/windowDrag.ts` on `pointerdown`. This explicit Tauri-native bridge is required for reliable macOS dragging when `src-tauri/tauri.conf.json` uses `titleBarStyle: "Overlay"` and `hiddenTitle: true`. The bridge calls `appWindow.startDragging()`, which requires the `core:window:allow-start-dragging` permission in `src-tauri/capabilities/default.json`. If that permission is missing, `startDragging()` will throw and the drag will silently fail.
...
**Preview surfaces:** `CollectionViewerPage` renders one of three surfaces based on `activeConfig.layout.preview`: `side-peek` → a resizable right rail (`Detail` with `surface="side-peek"`, default `sidePeekWidth: 440`), `bottom-peek` → a resizable bottom pane (`Detail` with `surface="bottom-peek"`, default `bottomPeekHeight: 280`), `full-page` → `FullPagePreview` hides list and shows nav strip. `Detail` (`src/views/collection/Detail.tsx`) accepts `surface`, persisted side/bottom size fields, `index`, `total`, `canMovePrevious/Next`, movement handlers, and `onResizeCommit` — renders `<aside aria-label="Issue detail">` with an accessible splitter (`role="separator"`). The side splitter uses `aria-orientation="vertical"`; the bottom splitter uses `aria-orientation="horizontal"`; arrow keys resize in small steps and Shift+Arrow resizes in larger steps. Resize commits patch only the relevant layout size field for the active named view.

**Preview measured-size contract:** Entity detail components receive optional `preview` metadata with `{ surface, width, height, sizeClass }`. The detail content frame also exposes `data-preview-surface`, `data-preview-size`, `--preview-width`, and `--preview-height`. Preview content should key compact/roomy decisions off measured size or `sizeClass`, not only off the selected surface name.
