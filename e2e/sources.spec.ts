...
test("Sources add Jira browser-only unavailable smoke path", async ({ page }) => {
  await page.goto("/");
  ...
  // The new JiraSourceForm wraps fields in <Form> with aria-label "Add Jira source".
  await expect(page.getByRole("form", { name: /add jira source/i })).toBeVisible();
  ...
  // reach the keychain or network. The browser fallback message is shown.
  await expect(
    page.getByText(/live connection testing is not available/i),
  ).toBeVisible();

  // Save the source — the primary action in the Form.Actions bar.
  await page.getByRole("button", { name: /^add source$/i }).click();
  ...
  await expect(page.getByText(/Kind: Jira · localhost/i)).toBeVisible();
