...
impl std::fmt::Display for AiError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AiError::MissingRoute { task_name } => {
                write!(f, "No AI profile is routed for {task_name}.")
            }
            AiError::MissingProfile { profile_name } => {
                write!(f, "AI profile not found: {profile_name}")
            }
            AiError::MissingEndpoint { endpoint_name } => {
                write!(f, "AI endpoint not found: {endpoint_name}")
            }
            AiError::MissingCredential { credential_name } => {
                write!(f, "AI credential not found: {credential_name}")
            }
            AiError::MissingSecret {
                credential_name,
                source,
            } => write!(
                f,
                "AI credential secret is missing for {credential_name} ({source})."
            ),
...
mod tests {
    ...
    fn redact_covers_api_key_header_name() {
        ...
        assert_eq!(
            redact("safe message about temperature"),
            "safe message about temperature"
        );
    }
}
