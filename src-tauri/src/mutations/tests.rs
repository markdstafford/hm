...
impl JiraMutationClient for RecordingJiraClient {
    ...
    fn create_issue_link(
        ...
        self.calls
            .lock()
            .unwrap()
            .push(format!("link:{}:{}:{}", source_key, target_key, link_type));
        let id = if self.null_link_id {
            None
        } else {
            Some(self.link_id.clone())
        };
        Ok(JiraCreatedIssueLink { id, self_url: None })
    }
...
mod cross_cutting_tests {
    ...
    use crate::mutations::inputs::{JiraUpdateTitleInput, MutationCommonInput};
    ...
    fn audit_write_failure_after_jira_success_returns_specific_error() {
        ...
        conn.execute_batch("DROP TABLE IF EXISTS audit_log")
            .unwrap();

        let client = RecordingJiraClient::default();
        let err = execute_jira_update_title(&conn, &client, make_title_input("AMP-1043", None))
            .unwrap_err();
        ...
        assert!(
            !debug.contains("secret-token-shaped-value"),
            "Debug leaked value: {debug}"
        );
    }
    ...
    fn jira_error_display_and_debug_do_not_contain_secrets() {
        ...
        let debug = format!("{err:?}");
        assert_eq!(
            display, "Jira mutation failed",
            "Display must be fixed safe string"
        );
        assert!(
            !debug.contains("secret-token"),
            "Debug must not leak secret-token: {debug}"
        );
        assert!(
            !debug.to_ascii_lowercase().contains("authorization"),
            "Debug must not leak Authorization: {debug}"
        );
        assert!(
            !debug.contains("raw body"),
            "Debug must not leak raw body: {debug}"
        );
        assert!(
    ...
    fn shared_batch_id_groups_multiple_entries() {
        ...
        execute_jira_update_title(&conn, &client, make_title_input("AMP-1", Some(batch_id)))
            .unwrap();
        execute_jira_update_title(&conn, &client, make_title_input("AMP-2", Some(batch_id)))
            .unwrap();
        execute_jira_update_title(&conn, &client, make_title_input("AMP-3", Some(batch_id)))
            .unwrap();
    ...
    fn omitted_batch_ids_are_unique_per_mutation() {
        ...
        let entry1 =
            execute_jira_update_title(&conn, &client, make_title_input("AMP-10", None)).unwrap();
        let entry2 =
            execute_jira_update_title(&conn, &client, make_title_input("AMP-11", None)).unwrap();

        assert_ne!(
            entry1.batch_id, entry2.batch_id,
            "separate calls without batch_id should produce unique batch ids"
        );
    }
    ...
    fn mutation_audit_state_does_not_contain_credential_shaped_values() {
        ...
        let entry = execute_jira_update_title(
            &conn,
            &client,
            JiraUpdateTitleInput {
                common: MutationCommonInput {
        ...
        for json in [&before_json, &after_json] {
            assert!(
                !json.to_ascii_lowercase().contains("authorization"),
                "audit state contains Authorization: {json}"
            );
            assert!(
                !json.contains("Bearer "),
                "audit state contains Bearer token: {json}"
            );
            assert!(
                !json.contains("credential_ref"),
                "audit state contains credential_ref: {json}"
            );
        }
    }
}
